...
Der Generator schreibt den Score-Verlauf während eines Laufs nach `.history/history.json` und exportiert ihn nach
`.dashboard/data/history.json`. Der Ordner `.history/` ist derzeit durch `.gitignore` ausgeschlossen und wird vom
Workflow nicht dauerhaft gespeichert. Deshalb enthält das veröffentlichte Dashboard aktuell nur die Historie, die im
jeweiligen Workflow-Lauf verfügbar ist. Die Begrenzung im Generator liegt bei 365 Einträgen.
