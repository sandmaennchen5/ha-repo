...
const reserved = new Set(['options.json', 'ingress-sessions', '.dockhand-storage.json', '.dockhand-storage.json.tmp', 'dockhand']);
...
export function prepareStorage(options, roots = { data: '/data', config: '/config', share: '/share' }) {
  ...
  const relative = location === 'share' ? (options.share_storage_directory ?? 'dockhand-config') : 'dockhand';
  ...
  const marker = path.join(roots.data, '.dockhand-storage.json');
  let source = roots.data;
  ...
  if (source !== target) {
    ...
    const names = fs.readdirSync(source).filter(name => source !== roots.data || !reserved.has(name));
    if (names.length) {
      ...
      // Do not delete stack bind-mount data belonging to running containers.
      ...
      console.log('Dockhand state copied to selected storage; previous location retained.');
    }
  }
