...
export function createIngressProxy({ backendPort = 3000, remember = false, sessionDirectory = '/data/ingress-sessions', allowedPeers = trustedPeers } = {}) {
  ...
  function requestHeaders(req, ctx) {
    ...
    // Drop every Dockhand session cookie before adding the current HA user's session.
  ...
  function responseHeaders(headers, ctx) {
    ...
    if (headers['set-cookie']) headers['set-cookie'] = headers['set-cookie'].map(cookie => {
      if (cookie.startsWith(cookieName + '=')) {
        ...
        if (remember) {
          if (value && /^[A-Za-z0-9_-]+$/.test(value) && Number.isFinite(expires) && expires > Date.now()) {
  ...
  const server = http.createServer((req, res) => {
    ...
    if (ctx.url === '/__dockhand_ingress.js') {
      res.writeHead(200, { 'content-type': 'application/javascript', 'cache-control': 'no-store' });
      res.end(runtime); return;
    }
    const headersToBackend = requestHeaders(req, ctx);
    if (ctx.url.split('?')[0] === '/api/auth/logout') fs.rmSync(ctx.file, { force: true });
    ...
    upstream.on('error', () => { if (!res.headersSent) res.writeHead(502); res.end('Dockhand backend unavailable'); });
