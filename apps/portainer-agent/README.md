...
- LTS- oder STS-Kanal auswählbar
- verschlüsselte Agent-API auf TCP-Port `9001`
- Zugriff auf die lokale Docker-Umgebung über die Home-Assistant-Docker-API
- optionales gemeinsames `agent_secret`
- keine Weboberfläche und keine eigenen Anwendungsdaten

## Einrichtung und Sicherheit

Schutzmodus deaktivieren, App starten und in Portainer eine Docker-Umgebung mit
der Adresse `<HOME-ASSISTANT-IP>:9001` anlegen. Auf Server und Agent muss
dasselbe `agent_secret` gesetzt sein. Port `9001` nur für den Portainer-Server
freigeben und niemals öffentlich erreichbar machen. Optionen für Protokoll,
Cluster, Adresse und Prüfintervalle stehen in [DOCS.md](DOCS.md).
