...
- LTS or STS channel selectable
- encrypted agent API on TCP port `9001`
- Access the local Docker environment via the Home Assistant Docker API
- optional shared `agent_secret`
- no web interface and no own application data

## Facility and Security

Deactivate protection mode, start the app and create a Docker environment in Portainer
the address `<HOME-ASSISTANT-IP>:9001`. Must be on server and agent
the same `agent_secret` must be set. Port `9001` for Portainer server only
release and never make it publicly available. options for protocol,
Cluster, address and check intervals are in [DOCS.md](DOCS.md).
