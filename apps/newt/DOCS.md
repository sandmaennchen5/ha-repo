...
## Dokumentation - Newt Client für Pangolin-Reverse-Proxy-Tunnel
...
3. Kopiere die generierte `id` und das `secret` aus den Site-Zugangsdaten.
4. Verwende dasselbe `endpoint`, das in deiner Pangolin-Installation konfiguriert ist.
...
- `id` und `secret` müssen zur selben Pangolin-Site gehören.
- Teile `secret` nicht mit unbefugten Personen.
...
- Stelle sicher, dass `endpoint` erreichbar ist und ein gültiges TLS-Zertifikat verwendet.
- Vergewissere dich, dass `id` und `secret` korrekt aus derselben Pangolin-Site stammen.
- Setze `extras.log_level` auf `debug`, um detailliertere Informationen zu erhalten.
