...
This fixed variant uses **Portainer Community Edition in the LTS channel**.
...
Administrator. `advanced.admin_password` is for initial creation only
thought. Direct ports are not required for Ingress. Details about
Locations, backups, ports and security are in [DOCS.md](DOCS.md).
